...
OSC_INDICATORS = ['UO', 'MACD', 'Stoch.RSI', 'Mom', 'BBP'] # Indicators to use in Oscillator analysis
OSC_THRESHOLD = 2 # Must be less or equal to number of items in OSC_INDICATORS
MA_INDICATORS = ['EMA10', 'EMA20', 'Ichimoku','VWMA', 'HullMA'] # Indicators to use in Moving averages analysis
...
#     print(f'Module Path: {os.path.dirname(os.path.realpath(__file__))}')
    ...
    for pair_name, analysis in multiple_anal.items():
        ...
        idx = pair_without_exchange.index(PAIR_WITH)
        kucoin_pair = pair_without_exchange[:idx] + "-" + pair_without_exchange[idx:] #a little mojo to help Kucoin know what's up
        oscilator_check = 0
        ...
        if DEBUG: print(f'Custsignalmod:{kucoin_pair} Oscillators:{oscilator_check}/{len(OSC_INDICATORS)} Moving averages:{moving_average_check}/{len(MA_INDICATORS)}')


        if oscilator_check >= OSC_THRESHOLD and moving_average_check >= MA_THRESHOLD:
            signal_coins[kucoin_pair] = kucoin_pair
#             print(f'Custsignalmod: Signal detected on {kucoin_pair} at {oscilator_check}/{len(OSC_INDICATORS)} oscillators and {moving_average_check}/{len(MA_INDICATORS)} moving averages.')
            signals_to_buy.append(kucoin_pair)
